...
require_once __DIR__ . '/../QuestionGenerator.php';

$fill_map = [
    'C Language' => ['Easy' => 22, 'Medium' => 22, 'Hard' => 22, 'Advanced' => 22, 'Expert' => 22],
    'Data Structure' => ['Easy' => 22, 'Medium' => 22, 'Hard' => 22, 'Advanced' => 22, 'Expert' => 22],
    'Java Programming Language' => ['Easy' => 20, 'Medium' => 22, 'Hard' => 22, 'Advanced' => 22, 'Expert' => 22],
    'PHP Programming Language' => ['Easy' => 22, 'Medium' => 22, 'Hard' => 22, 'Advanced' => 22, 'Expert' => 22],
    'Python Programming Language' => ['Easy' => 22, 'Medium' => 22, 'Hard' => 22, 'Advanced' => 22, 'Expert' => 22]
];

$totalInserted = 0;

function canonical_key(string $subject): string
{
    ...
    if (strpos($s, 'data structure') !== false) return 'ds';
    if ($s === 'c language' || $s === 'c') return 'c';
    return 'generic';
}

function explanation_for(string $key, string $topic): string
{
    return "This is correct because in {$key} fundamentals, {$topic} follows the standard language or data-structure rule.";
}

function mcq_templates(string $level): array
{
    return match ($level) {
        'Easy' => [
...
function concept_bank(string $key): array
{
    return match ($key) {
        'java' => [
...
function build_question(string $subjectName, string $level, int $seed): array
{
    $key = canonical_key($subjectName);
    $concepts = concept_bank($key);
    $templates = mcq_templates($level);

    $concept = $concepts[$seed % count($concepts)];
    $template = $templates[intdiv($seed, count($concepts)) % count($templates)];
    $question = sprintf($template, $concept['topic'], $subjectName) . " (Q" . ($seed + 1) . ")";
    ...
    return [
        ...
        'explanation' => explanation_for($subjectName, $concept['topic'])
    ];
}

function valid_question(array $q): bool
{
...
// Step 1: try QuestionGenerator first (compatibility probe)
try {
    $generator = new QuestionGenerator($conn);
    if (method_exists($generator, 'ensurePool')) {
        // Method exists, but legacy implementation targets text-subject schema.
        // Canonical insert path below is used for subject_id-based rows.
    }
} catch (Throwable $e) {
    // fall through to direct generator path below
}

$subjectIdStmt = $conn->prepare("SELECT id FROM subjects WHERE name = ? LIMIT 1");
$countStmt = $conn->prepare("SELECT COUNT(*) AS cnt FROM questions WHERE subject_id = ? AND level = ?");
$dupStmt = $conn->prepare("SELECT id FROM questions WHERE subject_id = ? AND level = ? AND question = ? LIMIT 1");
$insertStmt = $conn->prepare(
    "INSERT INTO questions (subject_id, level, type, question, option_a, option_b, option_c, option_d, correct_answer, explanation)
     VALUES (?, ?, 'MCQ', ?, ?, ?, ?, ?, ?, ?)"
);

foreach ($fill_map as $subjectName => $levels) {
    $subjectIdStmt->bind_param("s", $subjectName);
    $subjectIdStmt->execute();
    $subjectIdRes = $subjectIdStmt->get_result();
    $subjectRow = $subjectIdRes ? $subjectIdRes->fetch_assoc() : null;
    if (!$subjectRow) {
        echo $subjectName . ": subject not found, skipped" . PHP_EOL;
        continue;
    }

    $subjectId = (int)$subjectRow['id'];

    foreach ($levels as $level => $configuredNeed) {
        $countStmt->bind_param("is", $subjectId, $level);
        $countStmt->execute();
        $had = (int)$countStmt->get_result()->fetch_assoc()['cnt'];
        ...
        while ($needed > 0 && $current < 50 && $guard < 10000) {
            $guard++;
            $candidate = build_question($subjectName, $level, $seed++);
            if (!valid_question($candidate)) {
                continue;
            }
        ...
        echo $subjectName . " / " . $level . ": had " . $had . ", added " . $added . ", now " . $current . PHP_EOL;
    }
}

echo "Done. Total inserted: " . $totalInserted . PHP_EOL;
