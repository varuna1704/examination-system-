# Secure Enterprise Online Examination & Learning Management Suite: Unified Architecture, Specification & Visualization Manual
### The Comprehensive Solutions Architect Manual, Database Data Dictionary, and Process Visualization Catalog

---

## 1. Executive Summary & Platform Overview

The **Enterprise Online Examination & Learning Management Suite** is a high-security, role-based assessment platform engineered to deliver secure, scalably distributed, and highly interactive examinations. Built with a translucent glassmorphic dark-themed UI matching custom HSL color palettes, the system balances rigorous invigilation and anti-cheat mechanisms with advanced pedagogical instruments and multi-tenant class structures.

### Key Pillars of the System Architecture:
*   **Strict Role-Based Access Control (RBAC)**: Enforces complete physical separation between Student dashboards, Teacher management consoles, and Master Administrator configurations.
*   **Real-Time Proctoring Telemetry**: Employs an asynchronous client-server heartbeat syncing student statuses (pause, warn, terminate) in real time over a 3-second JSON polling cycle.
*   **Pedagogical Adaptivity**: Uses an Item Response Theory (IRT) difficulty placement engine to automatically shift question levels based on active student performance.
*   **Fail-Safe Offline Protection**: Intercepts standard browser connection drops, caching active exam choices in HTML5 LocalStorage, and automatically syncing data back to the database on network restoration.
*   **Curricular Analytics & Insights**: Aggregates exam completions into standard Bell-curve distributions, identifies the top 5 hardest questions to target syllabus gaps, and flags low-performing students requiring support.
*   **Cryptographic Verification Ceremony**: Seals passing attempts with an HMAC SHA-256 digital signature verification key, exposing verified accomplishments via a public, privacy-safe, gold-foil registry.

---

## 2. Platform Technology Stack Specification

The suite is engineered for absolute self-containment, high-concurrency event handling, and low-latency performance:

### A. Frontend Presentation Layer
*   **HTML5 (Semantic Layout)**: Restructures standard web templates using modern semantic tags (`<header>`, `<nav>`, `<main>`, `<section>`) to optimize search, accessibility, and device layout stability.
*   **Vanilla CSS3 (Layout & Styles)**:
    *   Employs custom CSS HSL variables to maintain design token uniformity.
    *   Leverages CSS Grid and Flexbox for high-fidelity responsive scaling from 320px mobile screens to wide monitors.
    *   Applies premium modern styles including glassmorphism (`backdrop-filter: blur()`), translucent boundaries, and smooth mouse-over micro-animations (`transform` transitions).
*   **Vanilla JavaScript (ES6+ App Logic)**:
    *   **Proctor Telemetry**: Orchestrates the 3-second AJAX background fetch polling loop syncing proctor states in real time.
    *   **Offline-Safe Sync**: Controls offline listeners (`online`/`offline` window events), local answer serialization, and AJAX batch-replay requests.
    *   **Interactive DOM Controls**: Handles code block monospaced rendering, timer count conversions, and grade distribution canvas drawings.

### B. Backend Application Layer
*   **PHP 8.x Engine (Hypertext Preprocessor)**:
    *   **Stateful Security**: Manages secure session tables (`$_SESSION`) validating active authentication tokens, authorization guards, and dynamic placement levels.
    *   **Type-Checked Database Queries**: Native `mysqli` driver bindings utilizing pre-compiled SQL prepared statements, neutralizing SQL injection vectors.
    *   **Algorithmic Calculators**: Processes Jaro-Winkler/Levenshtein similarity percentages, IRT placement math, and HMAC signature algorithms.

### C. Database Persistence Layer
*   **MySQL 5.7+ / MariaDB 10.x Relational Engine**:
    *   **Storage Engine**: Configured to InnoDB to enforce ACID transaction logs, row-level locking, and transactional isolation limits (`READ COMMITTED`).
    *   **Relational Integrity**: Enforces strict foreign keys with cascading deletions (`ON DELETE CASCADE`) to automatically purge orphaned attempts, answers, and memberships.

---

## 3. Normalized Database Schema & Data Dictionary

The platform's database architecture contains normalized structures separating directory, map, transaction, and gamification ledgers:
...
CREATE TABLE IF NOT EXISTS users (
    ...
    u_pass VARCHAR(255) NOT NULL, -- Cryptographically Hashed password via Bcrypt
    u_age INT NOT NULL,
...
-- 5. Cohort Membership Map (Enrolls Students to Classrooms)
...
-- 6. Cohort Scheduled Release Time Map (Binds Subjects to Class Calendars)
...
CREATE TABLE IF NOT EXISTS exam_attempts (
    id INT AUTO_INCREMENT PRIMARY KEY,
    verification_key VARCHAR(100) UNIQUE DEFAULT NULL, -- Cryptographic HMAC hash
    user_id INT NOT NULL,
    subject_id INT NOT NULL,
    level ENUM('Easy', 'Medium', 'Hard', 'Advanced', 'Expert') NOT NULL DEFAULT 'Medium',
...
> All mapping, attempts, and session-answer tables (`cohort_members`, `subject_cohorts`, `user_badges`, `exam_attempts`, `attempt_answers`) MUST incorporate explicit foreign key constraints referencing primary tables configured with `ON DELETE CASCADE`. Manual row orphan purging is strictly forbidden.

---

## 4. Step-by-Step Chronological Guide to Core Features

These steps represent the chronological journey an end-user traverses within the system workspace:

### Step 1: Secure Registration & Seeding
*   **User Action**: Candidates input legal metadata (Name, Username, Email, Age, Mobile, Address) inside `Registration.php`.
*   **Security Control**: Application hashes password inputs using dynamic Bcrypt default (`PASSWORD_DEFAULT`) prior to DB inserts.
*   **Admin Seeder**: Executing `seeds/seed_admin.php` generates the baseline privileged admin profile (User: `admin` | Pass: `admin123` | Role: `admin`).

### Step 2: Gateway Authentication & RBAC Routing
*   **User Action**: Logs in via `index.php` using Username and Password.
*   **Execution**: `Auth.php` extracts the verified user row, hashes the password check, and updates session arrays (`$_SESSION['user_id']`, `$_SESSION['role']`).
*   **Routing Logic**: Enforces routing gates; `student` profiles route to `subject.php`, `teacher` roles route to `admin_questions.php`, and `admin` profiles route straight to `admin_dashboard.php`.

### Step 3: Subject Directory Navigation
*   **User Action**: Enters `subject.php` to view academic offerings (e.g., Java, PHP).
*   **Validation**: The system queries active cohort membership tables. Cards display countdowns, closed warnings, or open launch actions based on current datetime vs. `subject_cohorts` bounds.

### Step 4: MCQ Exam Solver Session
*   **User Action**: Student enters `quiz.php` (graded official linear mode or AI adaptive mode).
*   **Control**: Employs background JavaScript polling, monitoring window blur events, and enforcing strict countdown closures.

### Step 5: Score Evaluation & Recording
*   **User Action**: Clicks submission or lets time expire.
*   **Backend Logic**: `result.php` processes results against `questions.correct_answer`, writes metrics, commits attempts records to database tables, and calculates gamification triggers.

### Step 6: Review & Self-Inspection
*   **User Action**: Inspects test corrections inside `review.php`.
*   **UI Breakdown**: Renders question blocks displaying explanations, highlighting incorrect selections in translucent red, and correct entries in green.

### Step 7: Competitive Leaderboard
*   **User Action**: Views global ranking podiums inside `leaderboard.php`.
*   **Ordering Algorithm**: Ranks candidates based on peak correct percentage, breaking ties using speed duration.

---

## 5. Architectural Blueprints & Process Diagrams Catalog

This section houses all **31 master diagrams** detailing the system workflows.

---

### Division 1: Project Management Diagrams
...
### Division 2: Process & Workflow Diagrams
...
### Division 3: Technical & System Design Diagrams
...
#### D. Data Flow Diagram (DFD): Level-1 Information Flow Dynamics
...
### Division 4: Advanced Security, Runtime States & Logic Diagrams
...
flowchart TD
    ...
    QueryPeers & JW_Peer & Lev_Peer --> CalcPeerScore[Compute average peer-to-peer similarity percentage]
...
## 6. Functional Outlines & Security Matrices

To ensure robust data-integrity and security boundaries, the platform enforces compliance mappings across dynamic tiers:

### A. Role-Based Permission Scope Matrix
The platform separates roles to ensure strict multi-tenant boundaries:

| Entity Resource | Student Role | Teacher Role | Administrator Role |
| :--- | :--- | :--- | :--- |
| **Questions Directory** | Read-Only (within active exam contexts) | Create, Read, Update, Delete (CRUD) | CRUD Operations |
| **User Access Catalog** | Denied Access | Read-Only (Performance metrics) | Create, Read, Update, Delete (CRUD) |
| **Cohort & Calendar Maps** | Read-Only (Dashboard visibilities) | Read-Only (Cohort grades reports) | CRUD Operations |
| **Plagiarism Flags Ledger** | Denied Access | Read & Evaluate Warnings | Read & Evaluate Warnings |
...
### B. Proctor Status & Warning State Limits
Heartbeats track candidate telemetry states through designated thresholds:
*   `monitoring`: Standard interactive exam solving.
*   `warning_1`: Initial focus loss/tab shifting event detected. Injects a sliding orange overlay.
*   `warning_2`: Subsequent focus loss event detected. Displays warning toast.
*   `suspended`: Proctor triggers pause. Countdown freezes and screen is blurred.
*   `completed`: Terminated by proctor. Answers are compiled, scored, and session ends.

---

## 7. Business Continuity, DevOps & Disaster Recovery Playbook

This DevOps guide establishes recovery targets, build lifecycles, and database procedures:

### A. Service Level Recovery Targets
*   **Recovery Point Objective (RPO)**: Target data-loss bounds under node crashes is **< 3 seconds** (guaranteed by LocalStorage buffering and heartbeat sync intervals).
*   **Recovery Time Objective (RTO)**: Target recovery to full platform functionality is **< 5 minutes** (enabled by active-active server setups).

### B. Automated CI/CD Release Pipeline
```mermaid
flowchart TD
    DevCommit[GitHub Code Commit] --> WebhookTrigger[CI Webhook Trigger]
    WebhookTrigger --> CI_Server[GitHub Actions / Jenkins Build]
    
    subgraph Pipeline Security Validations
        CI_Server --> PHP_Lint[PHP CodeSniffer]
        CI_Server --> PHP_Unit[PHPUnit Security Tests]
        CI_Server --> SchemaCheck[MariaDB Migration Test]
    end

    PHP_Lint & PHP_Unit & SchemaCheck -->|Success| PackArchive[Package Docker/EC2 Artifacts]
    PackArchive --> RollingDeploy[Zero-Downtime Rolling Update]
    RollingDeploy --> ProductionWebs[AWS EC2 Active Clusters]
```

### C. Database Backup & Pruning Schedule
*   **Incremental Backups**: Automated hourly MariaDB replication backups to off-site S3 storage.
*   **Archival Pruning**: Active scheduler running standard database scrubbing scripts to archive attempts older than 18 months, maintaining high execution speeds.
...
## 8. Presentations & Evaluator Walkthrough Guide

Use this strategic framework to explain the platform to developers, teachers, or evaluators:

### 1. Highlight Technical Foundations (RBAC & Stack)
*   **Script Links**: [index.php](file:///d:/gitpush%20project/Examination_System_Project/Exam/index.php) and [Auth.php](file:///d:/gitpush%20project/Examination_System_Project/Exam/Auth.php).
*   **Architectural Point**: *"We built a fully secured, enterprise-grade assessment engine. When a student logs in, they see their subjects and scheduled classes (Cohorts). An administrator or teacher enters a centralized dashboard to manage catalog subjects, adjust user permissions dynamically (Student, Teacher, Admin), and construct new question pools with syntax previews."*

### 2. Demonstrate Proctoring Integrity (Telemetry)
*   **Script Links**: [proctor_console.php](file:///d:/gitpush%20project/Examination_System_Project/Exam/proctor_console.php) and [proctor_check.php](file:///d:/gitpush%20project/Examination_System_Project/Exam/proctor_check.php).
*   **Architectural Point**: *"To prevent cheating, we developed a live invigilator dashboard. The proctor can monitor student progress. By polling a heartbeat script every three seconds in the student's browser, the system allows the proctor to issue warnings, pause the exam countdown, or force-submit the test instantly."*

### 3. Discuss Adaptive Pedagogy (Adaptivity & Gamification)
*   **Script Links**: [adaptive_quiz.php](file:///d:/gitpush%20project/Examination_System_Project/Exam/adaptive_quiz.php) and [skills_passport.php](file:///d:/gitpush%20project/Examination_System_Project/Exam/skills_passport.php).
*   **Architectural Point**: *"Instead of standard tests, the platform features an AI Adaptive Placement Mode. The system gauges the candidate's actual skill level by adjusting subsequent question difficulties in real time based on their correctness. At the end, the system averages their steady-state difficulty to unlock digital skills badges (like 'PHP Master' or 'Java Artisan') on their profile page."*

### 4. Detail Resiliency & Credential Safety (Offline Sync & Certificates)
*   **Script Links**: [sync_answer.php](file:///d:/gitpush%20project/Examination_System_Project/Exam/sync_answer.php) and [verify.php](file:///d:/gitpush%20project/Examination_System_Project/Exam/verify.php).
*   **Architectural Point**: *"If a candidate's internet drops, they don't lose progress. A local HTML5 worker automatically saves their choices in LocalStorage and syncs them back to the database once connection is restored. Upon passing, a cryptographically signed verification key is generated, allowing external employers to verify their credentials on our public certificate registry page."*
